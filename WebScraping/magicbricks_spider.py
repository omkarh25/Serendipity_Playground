...
# def convert_price(price_str):
#     price_str = price_str.replace('₹', '').replace('L', '').replace('Cr', '').strip()
#     if 'Cr' in price_str:
#         return int(float(price_str) * 1e7)
#     elif 'L' in price_str:
#         return int(float(price_str) * 1e5)
#     return int(price_str)

# # Function to clean and extract built-up area
# def convert_area(area_str):
#     area_str = area_str.replace('sq.ft.', '').strip().replace(',', '')
#     return int(area_str)
...
#     # Scrape a certain number of pages (or until there is no more data)
...
#             try:
#                 location = listing.find('h1', class_='css-1hidc9c').text
#                 price = listing.find('span', class_='css-19rl1ms').text
#                 area = listing.find('div', class_='_h3yh40').text
...
#                 # Print each listing's data for debugging
#                 print(f"Added data: Location: {location}, Price: {formatted_price}, Area: {formatted_area}")
...
# # Print out the dataframe to see if it contains data
# print(df)

# # Directory and file path for the Excel file
# excel_dir = 'C:/Users/91861/Desktop/Bhoodevi'
# excel_file = 'housing_data.xlsx'
...
# Set the number of pages you want to scrape
max_pages = 150  # Reduced for demonstration, change as needed
...
excel_file_path = "C:\\Users\\91861\\OneDrive\\Desktop\\bhoodevi\\WebScraping\\Apartmentcomm.xlsx"
...
try:
    ...
    alternative_path = "C:\\Users\\91861\\Documents\\apartmentcomm.xlsx"
    try:
